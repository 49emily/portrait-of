...
function UserSection({ user, plaqueName, API_BASE_URL }) {
  ...
  const viewedPortrait = history.length > 0 ? history[0] : null;
    ...
    <div className="min-h-screen flex flex-col items-center p-6 md:p-8">
      <header className="w-full max-w-6xl text-center mb-10">
        ...
        <p className="text-lg text-gray-300 max-w-2xl mx-auto">
          A living artwork that evolves with your digital habits. Each portrait
          reflects the hidden cost of unproductive screen time, capturing the
          slow transformation of identity in the age of distraction.
        </p>
      </header>
      ...
      <main className="w-full max-w-6xl flex flex-col gap-16">
        <UserSection
          user="justin"
          plaqueName="Justin Guo"
          API_BASE_URL={API_BASE_URL}
        />
        <UserSection
          user="emily"
          plaqueName="Emily Zhang"
          API_BASE_URL={API_BASE_URL}
        />
      </main>
    </div>
